...
if ($_GET) {
// check url to determin which key in get
    if (isset($_GET['sub'])) {
        if (is_numeric($_GET['sub'])) {
            $subCategoryObject->setId($_GET['sub']);
            $findSubResult = $subCategoryObject->findSubcategoryById();
            if ($findSubResult) {
        ...
        if (is_numeric($_GET['brand'])) {
            ...
            $findBrandResult = $brandObject->findBrandById();
            if ($findBrandResult) {
                $productObject->setBrand_id($_GET['brand']);
                // fetch product according brand
                $productResult = $productObject->getProductByBrands();
            } else {
                header('location:views/errors/404.php');
                die;
            }
